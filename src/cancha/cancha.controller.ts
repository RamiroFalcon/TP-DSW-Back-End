...
export class CanchaController {
  ...
  crear = async (req: Request, res: Response): Promise<void> => {
    try {
      const cancha = this.service.crearCancha(req.body);
      res.status(201).json({
        success: true,
        data: cancha,
  ...
  obtenerTodas = async (req: Request, res: Response): Promise<void> => {
    try {
      const canchas = this.service.obtenerTodas();
      res.status(200).json({
        success: true,
        data: canchas
      });
  ...
  obtenerPorId = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const cancha = this.service.obtenerPorId(id);
      res.status(200).json({
        success: true,
        data: cancha
      });
  ...
  obtenerPorTipo = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const canchas = this.service.obtenerPorTipo(id_tipo);
      res.status(200).json({
        success: true,
        data: canchas
      });
  ...
  actualizar = async (req: Request, res: Response): Promise<void> => {
    try {
      const id = parseInt(req.params.id);
      const cancha = this.service.actualizarCancha(id, req.body);
      res.status(200).json({
        success: true,
        data: cancha,
  ...
  eliminar = async (req: Request, res: Response): Promise<void> => {
    try {
      const id = parseInt(req.params.id);
      this.service.eliminarCancha(id);
      res.status(200).json({
        success: true,
